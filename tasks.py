...
class TravelTasks:
    ...
    def plan_itinerary(self, agent, city, travel_date, interests):
        return Task(
            description=dedent(
                f"""
            **Task**: Plan a 7-Day Detailed Itinerary
            
            **Description**: Expand the city guide into a detailed 7-day itinerary with per-day plans, including weather forecasts, places to eat, packing suggestions, and a budget breakdown. You MUST suggest actual places to visit, actual hotels to stay, and actual restaurants that are still on business to go to.
            ...
            **Note**: {self.__tip_section()}    
            ...
            agent=agent,
            expected_output="A detailed 7-day itinerary with per-day plans, including weather forecasts, places to eat, packing suggestions, and a budget breakdown."
        )
    ...
    def gather_city_info(self, agent, city, travel_dates, interests):
        return Task(
            description=dedent(
                f"""
                **Task**: Gather Detailed Information on {city}
            ...
            agent=agent,
            expected_output="An in-depth guide for the selected city, including key attractions, local customs, special events, and daily activity recommendations."
        )